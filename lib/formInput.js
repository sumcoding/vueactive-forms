...
const setInputObject = (value) => ({ [FormKey.INVALID]: null, [FormKey.DIRTY]: null, [FormKey.ORIGINAL]: value, value });
...
const setInput = (input$, validations) => {
  ...
  Object.entries(validations).map(([key, obj]) => input$[key].value = obj);

  /** Set input to dirty */
  input$[FormKey.DIRTY].value = input$.value.value !== (null || undefined || '');
}
